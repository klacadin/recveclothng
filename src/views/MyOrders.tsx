...
interface Order {
  id: string;
  order_number: string;
  status: 'new' | 'pending_payment' | 'for_verification' | 'paid' | 'preparing' | 'packed' | 'for_pickup' | 'shipped' | 'completed' | 'cancelled' | 'failed';
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        const data = email
          ? await apiGet<Order[]>(`/api/orders?email=${encodeURIComponent(email)}`)
          : [];
        setOrders(data || []);
      } catch (error) {
                ...
                <Card className="sticky top-4">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {(selectedOrder.status === 'new' || selectedOrder.status === 'pending_payment' || selectedOrder.status === 'for_verification') &&
                      ...
                      !selectedOrder.hitpay_payment_id && (
                    ...
                    <div>
                      ...
                      {selectedOrder.hitpay_payment_id && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Payment ID: <span className="font-mono">{selectedOrder.hitpay_payment_id}</span>
                        </p>
                      )}
                      {selectedOrder.hitpay_payment_id && selectedOrder.status === 'paid' && (
