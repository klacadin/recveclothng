...
const Checkout = () => {
  ...
  const handleOTPVerified = async () => {
    ...
    try {
      ...
      const createdOrderId = data?.order?.id || data?.order_id;
      if (!createdOrderId) {
      ...
      toast({
        ...
        description: `Your order number is ${data.order?.order_number || data.order_number}. We'll contact you soon.`,
      });
      navigate('/order-confirmation', {
        state: {
          orderNumber: data.order?.order_number || data.order_number,
          paymentMethod: 'cod',
        },
      });
    } catch (error) {
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (!res.ok) {
                                throw new Error(
                                  data?.message || 'Voucher service temporarily unavailable'
                                );
                              }
                              if (data?.valid) {
